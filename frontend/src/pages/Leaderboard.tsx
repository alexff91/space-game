...
/**
 * ПОЧЕМУ убран запасной набор данных: раньше и в демо-режиме, и при любой
 * ошибке запроса страница показывала зашитый список — двенадцать
 * несуществующих людей во главе с вымышленным лидером и 12 400 очками. Человек видел
 * своё место в этом «рейтинге». Теперь пустой ответ и ошибка выглядят как
 * то, чем они являются: данных нет.
 */
export default function Leaderboard() {
  ...
  const loadLeaderboard = async () => {
    ...
    if (isDemoMode()) {
      setUsers([]);
      setError('There is no backend in this build, so no scores are recorded and there is no ranking to show.');
      setLoading(false);
      return;
    }
    try {
      ...
      setUsers([]);
      setError('The leaderboard could not be loaded from the server.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {users.length === 0 ? (
        <div className="card text-center py-16">
          <div className="text-3xl font-bold text-gray-500 mb-3">No data</div>
